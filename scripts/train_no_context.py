...
class BertForSequenceLabeling(nn.Module):
    ...
    def __init__(self, model_dir, config, freeze_bert=False):
...
def load_model(model_type, pretrained_dir, checkpoint_file, num_labels, freeze_bert):
...
def main():
    ...
    model = load_model(args.model_type, args.pretrained_dir, args.checkpoint_file, len(tagset), args.freeze_bert)
    model.to(device)
    ...
    if mode == 'train':
        ...
        best_model = load_model(args.model_type, None, best_model_dir, len(tagset), args.freeze_bert)
