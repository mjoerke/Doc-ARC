...
class BertWithAttention(nn.Module):
    ...
    def __init__(self, model_dir, config, freeze_bert=False, lstm_hidden_dim=128, vanilla=False):
        ...
        if not vanilla:
            self.context_encoder = nn.LSTM(4 * self.BERT_DIM, self.LSTM_HIDDEN_DIM, bidirectional=True, batch_first=True)
    ...
    def get_static_context_representation(self, data, batch_size):
        context_inputs = data['context_sequences'].to(device)
        context_masks = data['context_masks'].to(device)
        context_transforms = data['context_transforms'].to(device)

        N, L_words, L_toks = context_transforms.shape
        context = torch.zeros((N, L_words, 4 * self.BERT_DIM))
        # context.to(device)
        ...
        with torch.no_grad():
            for b in range(num_batches):
                ...
                context[start:stop, :] = self.forward_bert(context_inputs[start:stop], attention_mask=context_masks[start:stop], 
                                                    transforms=context_transforms[start:stop]).cpu()
        return context

    def forward(self, inputs, masks, transforms, full_context, attn_sentence_idx, attn_word_idx, attn_dists, attn_mask):

        inputs = inputs.to(device)
        masks = masks.to(device)
        transforms = transforms.to(device)

        attn_sentence_idx = attn_sentence_idx.to(device)
        attn_word_idx = attn_word_idx.to(device)
        attn_dists = attn_dists.to(device)
        ...
        # attn_sentence and _word_idx are batch_size x max_seq_len x k
        # index full_context representation table using unraveled matrix
        full_context_representations = full_context[attn_sentence_idx.view(B * L * K), attn_word_idx.view(B * L * K)]
        # reshape back to original
        full_context_representations = full_context_representations.view(B, L, K, -1)

        if not self.vanilla:
            encoded_context_representations = torch.zeros((B, L, K, 2 * self.LSTM_HIDDEN_DIM)).to(device)

            for k in range(K):
                encoded_context_representations[:, :, k, :], _ = self.context_encoder(full_context_representations[:, :, k, :].to(device))

            encoded_context_representations = encoded_context_representations.view(B * L, K, -1)
        else:
            encoded_context_representations = full_context_representations.view(B * L, K, -1).to(device)

        dist_embeds = self.dist_embeddings(attn_dists.view(B * L, K))
        attention = torch.cat((encoded_context_representations, dist_embeds), -1)
    ...
    def evaluate(self, data_loader, batch_size, metric, tagset, k):
        ...
        with torch.no_grad():
            ...
            for document_batch in data_loader:
                ...
                context = self.get_static_context_representation(batches, batch_size)
                num_batches = len(batches['inputs'])

                for b in range(num_batches):

                    logits = self.forward(inputs=batches['inputs'][b], masks=batches['masks'][b], transforms=batches['transforms'][b], 
                                            full_context=context,
                                            attn_sentence_idx=batches['attn_sentence_idx'][b], 
                                            attn_word_idx=batches['attn_word_idx'][b],
                                            attn_dists=batches['attn_dists'][b],
                                            attn_mask=batches['attn_masks'][b])
        ...
        with open(pred_file, 'w+', encoding='utf8') as f:
            ...
            with torch.no_grad():
                ...
                for document_batch in data_loader:
                    ...
                    print(doc_id)
                    f.write(doc_id + "\n")
                    batches = get_batches(document, batch_size=16, tagset=tagset, num_attended=10)

                    context = self.get_static_context_representation(batches, batch_size=16)
                    num_batches = len(batches['inputs'])
...
def get_batches(document, batch_size, tagset, num_attended):
    ...
    for b in range(num_batches):
        ...
        batched_attn_sentence_idx.append(torch.LongTensor(attn_sentence_idx))
    ...
    num_attended_sequences = len(document.all_attended_seq)
    # length in # tokens
    max_context_sequence_len = max([len(document[i].token_ids) for i in document.all_attended_seq])
    # length in # words after transform
    max_context_sentence_len = max([len(document[i].words) for i in document.all_attended_seq])

    context_sequences = np.zeros((num_attended_sequences, max_context_sequence_len))
    ...
    for i in range(num_attended_sequences):
        s = document[document.all_attended_seq[i]]

        n_tokens = len(s.token_ids)
        n_words = len(s.words)

        context_sequences[i, :n_tokens] = s.token_ids
        context_masks[i, :n_tokens] = [1.0] * n_tokens
        context_transforms[i, :n_words, :n_tokens] = s.transform

    context_sequences = torch.LongTensor(context_sequences)
    context_masks = torch.FloatTensor(context_masks)
    context_transforms = torch.FloatTensor(context_transforms)
