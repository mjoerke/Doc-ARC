...
def check_span_f1_two_dicts(gold, pred):
	...
	logging.info ("precision: %.3f %s/%s" % (precision, cor, len(pred)))
	logging.info ("recall: %.3f %s/%s" % (recall, cor, len(gold)))
	logging.info ("F: %.3f" % F)
...
def check_span_f1_two_lists(gold, preds, orig_tagset):
	...
	logging.info ("precision: %.3f %s/%s" % (p, correct, trials))
	logging.info ("recall: %.3f %s/%s" % (r, correct, trues))
	logging.info ("F: %.3f" % f)
